...
class Robot:
    ...
    def think(self, new_area, robot_pos_in_area):
        ...
        if not self.target_found and self.target_position is not None:
            ...
            for i in range(len(self.displacement_row)):
                # line below: storing robot row position as a result of row displacement
                row = self.robot_current_position[0] + self.displacement_row[i]
                # line below: storing robot column position as a result of column displacement
            ...
            if not empty_cell_found:  # if no empty cell found
                if len(self.movement_stack) > 0:
                    ...
                    # were stored as negative in the movement stack on line 127 (in order for the agent to move back accordingly
                    # from where it came from (backtrack) until an empty and un-visited cell is found
    ...
    def think_smart(self, new_area, robot_pos_in_area):
        ...
        # below is a for loop to check if the target is found in any of the surrounding cells as due to the action
